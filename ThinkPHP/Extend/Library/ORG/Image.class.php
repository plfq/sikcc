...
class Image {
    ...
    // 中文验证码
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='SourceCodePro-Bold.ttf', $verifyName='verify') {
        import('ORG.String');
        $code = String::randString($length, 5);
        $width = ($length * 45) > $width ? $length * 45 : $width;
        session($verifyName, md5(strtolower($code)));
        $im = imagecreatetruecolor($width, $height);
        $borderColor = imagecolorallocate($im, 100, 100, 100);                    //边框色
        ...
        for ($i = 0; $i < 20; $i++) {
			...
			imagestring($im, mt_rand(1, 4), $i * 10 + 5, mt_rand(1, 40), String::randString(1, 5), imagecolorallocate($im, mt_rand(100, 220), mt_rand(100, 220), mt_rand(100, 220)));
        }
		imagesetthickness($im,6);
		imagearc($im, mt_rand(-10, $width), mt_rand(-10, $height), mt_rand(30, 300), mt_rand(20, 200), 65, 44, imagecolorallocate($im, mt_rand(100, 255), mt_rand(100, 255), mt_rand(100, 255)));
		if (!is_file($fontface)) {
            $fontface = dirname(__FILE__) . "/" . $fontface;
        }
        for ($i = 0; $i < $length; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0, 120)); //这样保证随机出来的颜色较深。
            $codex = String::msubstr($code, $i, 1,"utf-8",  mt_rand(1, 10)>5);
            imagettftext($im, mt_rand(24, 30), mt_rand(-60, 60), 40 * $i + 20, mt_rand(30, 35), $fontcolor, $fontface, $codex);
        }
        Image::output($im, $type);
    }
